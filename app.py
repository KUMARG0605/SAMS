...
# Connect directly to SAMS database
conn = pyodbc.connect(
    'DRIVER={SQL Server};'
...
def student_login():
    ...
    select_query = 'SELECT id_number, email, password, semister, branch FROM registration WHERE email = ?'
    cursor.execute(select_query, (email,))
    result = cursor.fetchone()

    if result and check_password_hash(result[2], password):
        # User exists with the provided email and password is verified
        id_number, email, hashed_password, semister, branch = result
...
def send_password_reset_email(email, reset_token):
    ...
    email_address = "indexlearners03@gmail.com"
    email_password = "462513641582@k"
...
def reset_password(reset_token):
    if request.method == 'POST':
        new_password = generate_password_hash(request.form['new_password'])  # Hash the new password
...
def faculty_login():
    ...
    select_query = 'SELECT id_number, password, branch FROM faculty_data WHERE Id_number = ?'
    cursor.execute(select_query, (faculty_id,))
    result = cursor.fetchone()
    ...
    if result is not None and check_password_hash(result[1], password) and faculty_subjects:
